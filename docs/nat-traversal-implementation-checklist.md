...
3. Finish removing `base_url` plus `device_token`-shaped app models.
   Remaining work: mostly naming and compatibility cleanup now. Persisted client state is identity-first rather than URL-plus-token-first, Android/Windows/iOS are on bootstrap-aware connection inputs, and the client enrollment / direct-client path no longer carries `device_token` or bearer-token auth compatibility in the main Rust stack.
