...
| Managed rendezvous failover package JSON | Export/import packages should keep top-level fields `version`, `cluster_id`, `source_node_id`, `target_node_id`, `exported_at_unix`, `public_url`, `pbkdf2_rounds`, `salt_b64`, `nonce_b64`, and `ciphertext_b64` stable | Failover handoff files become an operator disaster-recovery contract |
