...
- A relay ticket must negotiate `inner_mtls`; production clients and nodes fail closed when the peer identity, cluster binding, certificate chain, or security mode is invalid. There is no plaintext fallback for these paths.
